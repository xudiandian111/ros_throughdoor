...
void Bebop::generalControl() //常规控制
{
    ...
    */
    if (Parameter::turn_left_flag == false && Parameter::turn_right_flag == false)
    {
        //getControl.checkStatus();
    }
...
#ifdef TEST
    ROS_INFO("speed:%lf,dis:%lf,angle:%lf,erro_angle:%lf/n", control_parameter[0][0], control_parameter[0][1], control_parameter[1][2],erro_angle);
#endif
...
void Bebop::rosVersion()
{
    ...
    image_sub = it.subscribe("/bebop/image_raw", 1, &Detections::imagePcs, &getDetections);
    ...
    while (ros::ok())
    {
        ...
        //ROS_INFO("speed:%lf",ardrone_speed);
